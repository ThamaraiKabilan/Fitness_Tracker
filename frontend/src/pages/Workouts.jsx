...
import { Box, Typography, Paper, IconButton, Avatar, Card, TextField, Chip, Stack, InputAdornment } from '@mui/material';
import { motion, AnimatePresence } from 'framer-motion';
import { exerciseDatabase } from '../services/exerciseData';
import API from '../services/api';
import { ThemeContext } from '../context/ThemeContext';
import { FilterContext } from '../context/FilterContext';
import { CheckCircle, PlayArrow, Pause, Stop, Delete, Info, Stars, History, Lock, EventNote, Search } from '@mui/icons-material';

const Workouts = () => {
  const { darkMode } = useContext(ThemeContext);
  const { dateFilter } = useContext(FilterContext);
  
  // FIXED: Declaring all necessary states
  const [selectedExercises, setSelectedExercises] = useState([]);
  const [sessionName, setSessionName] = useState("Daily Workout"); // This line fixes the error
  ...
  const toggleComplete = (exIdx, sIdx) => {
    ...
    // Dynamic Calorie Calculation: (Weight * Reps * 0.1)
  ...
  const finishWorkout = async () => {
    if (selectedExercises.length === 0) return alert("Please add an exercise first!");
    ...
    try {
      ...
      setSessionName("Daily Workout");
      fetchWorkouts();
    } catch (err) { 
        console.error(err);
        alert("Error saving session. Check backend terminal."); 
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <Box mb={4} display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
            <Box>
                ...
                <Typography color="textSecondary">{isToday ? "Log your live activity" : `Reviewing history for ${dateFilter}`}</Typography>
            </Box>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
                <Paper sx={{ p: 0, mb: 4, bgcolor: 'background.paper', borderRadius: 6, overflow: 'hidden' }} className="border shadow-sm">
                  <Box p={3} pb={1}>
                    ...
                    <TextField 
                        fullWidth size="small" placeholder="Search exercises..." sx={{ mb: 2 }} 
                            ...
                            <Box key={sub} py={1} borderBottom="1px solid rgba(0,0,0,0.05)">
                              ...
                              {exerciseDatabase[cat][sub].filter(ex => ex.name.toLowerCase().includes(searchTerm.toLowerCase())).map(ex => (
                                <Box key={ex.name} display="flex" justifyContent="space-between" alignItems="center" px={1} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                                  <Button variant="link" className="text-start py-2 text-decoration-none" style={{ color: darkMode ? '#fff' : '#000', fontSize: '0.9rem' }} 
                                    onClick={() => setSelectedExercises([...selectedExercises, { ...ex, sets: [{ kg: '', reps: '', completed: false }], caloriesBurned: 0 }])}>
                                      + {ex.name} {ex.level === userLevel && <BBadge bg="info" className="ms-2" style={{fontSize:'0.5rem'}}>SUGGESTED</BBadge>}
                                  </Button>
                ...
                <Paper sx={{ p: 5, textAlign: 'center', bgcolor: 'background.paper', borderRadius: 6, border: '1px dashed #ccc' }}>
                    ...
                    <Typography variant="h6" fontWeight="700">Past Records</Typography>
                    <Typography variant="body2" color="textSecondary">You cannot log new data for past dates.</Typography>
                </Paper>
          )}
        </Col>

        {/* Right Section: Active Tracker */}
        <Col lg={8}>
          <AnimatePresence>
          ...
          <Box mt={5}>
            ...
            {history.map((session) => (
                <Paper key={session._id} sx={{ p: 3, mb: 2, bgcolor: 'background.paper', borderRadius: 4 }} className="border shadow-sm">
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                        <Box>
